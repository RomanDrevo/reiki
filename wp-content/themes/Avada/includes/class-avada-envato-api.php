...
if ( ! class_exists( 'Avada_Envato_API' ) ) :
	...
	class Avada_Envato_API {
		...
		/**
		 * Main Avada_Envato_API Instance
		 *
		 * Ensures only one instance of this class exists in memory at any one time.
		 *
		 * @see Avada_Envato_API()
		 * @uses Avada_Envato_API::init_globals() Setup class globals.
		 * @uses Avada_Envato_API::init_actions() Setup hooks and actions.
		 *
		 * @since 1.0.0
		 * @static
		 * @return object The one true Avada_Envato_API.
		 * @codeCoverageIgnore
		 */
		...
		private function init_globals() {
			// Envato API token.
			self::$token = Avada()->registration->get_token();
		}
		...
		public function request( $url, $args = array() ) {
			$defaults = array(
				'headers' => array(
					...
					'User-Agent' => 'WordPress - Avada ' . Avada()->get_theme_version(),
				),
				'timeout' => 20,
			);
